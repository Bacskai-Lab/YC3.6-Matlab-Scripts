function YC_Pseudocolor(s, r_min, r_max, brightness_factor, median_filter, xbitdepth)
...
for j = 1:length(s)
    ...
    for k = 1:length(meas_array)
        ...
        filelist = dir(sprintf('%s/%s.oif.files/s_C001T*.tif', INPUT_PATH, meas_name));
        if length(filelist) > 0
            type = 1; % Time Courese
            num_meas = length(filelist);
        end
        
        if length(slice_array) == 0
            idx_array = 1:num_meas;
        else
            idx_array = slice_array;
        end
        
        for l = idx_array
            if type == 0
                temp1 = double(imread(sprintf('%s/%s.oif.files/s_C001Z%03d.tif', INPUT_PATH, meas_name, l)));
                temp2 = double(imread(sprintf('%s/%s.oif.files/s_C002Z%03d.tif', INPUT_PATH, meas_name, l)));
            elseif type == 1
                temp1 = double(imread(sprintf('%s/%s.oif.files/s_C001T%03d.tif', INPUT_PATH, meas_name, l)));
                temp2 = double(imread(sprintf('%s/%s.oif.files/s_C002T%03d.tif', INPUT_PATH, meas_name, l)));
            end
